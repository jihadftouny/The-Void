...
3. `npm run build` — succeeds or FAIL. If this unit adds or changes the runtime entry point, **boot it for real** (start `npm run dev` / load the built entry) and confirm it starts without crashing — a green build and a green test-runner suite can both hide a real-entry crash from a missing global or an environment difference.
