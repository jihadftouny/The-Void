...
- **⚠ A SECOND ESCAPE from this unit, found by the author in play on 2026-09-12:** *"floor 3 and 2 are only lines in the background, no gradient colors or anything."* Floors 1 and 4 got soft radial gradients; floors 2 and 3 got thin repeating 1 px lines at 7% on near-black. Floor 2's *"blinding white"* had been traded for red's readability and hidden in lines that do not read as white; floor 3's crosshatch had no such constraint and was simply weak. *Which agent should have caught it:* none could by test — contrast gates prove readability, not fidelity to a direction; it was correctly a human check. **What the pipeline could have done:** the build agent's report said floor 2's white *"went into the ink and the texture"*, and the orchestrator relayed that without asking whether 7% lines would read as white. Fixed in unit `floor-looks` (floor 2 becomes a light floor; floor 3 a grey haze with ash).
