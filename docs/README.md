...
| 12 | **[BALANCE-REPORT.md](BALANCE-REPORT.md)** | *Whether it is winnable.* Generated from the sim — do not hand-edit. ⚠ **INVALIDATED 2026-08-28** — it calls itself a no-equipment *lower bound*, but `FINDINGS.md` G11 shows loot is un-equippable **in principle**, so those figures are what real play does, not a floor beneath it. **Regenerate after `PLAN.md` #0** | touching any balance constant |
