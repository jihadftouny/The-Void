...
| 10 | **[SHIP-SCOPE.md](SHIP-SCOPE.md)** | ⭐ **The gate against scope creep.** The **v1 test** (§2.1 — three questions that decide whether *anything*, including something nobody has thought of yet, belongs in v1), the **≤60 h scope**, the cut list with **a version against every cut row**, the **increment ladder** (§9 — v1.1→v2.0), the pre-armed cut ladder, and the **two conflicts with the Ftouny Plan v3 rules**. Selects from `PLAN.md`; does not replace it | committing to any work item, **and before proposing any new feature** — run §2.1 first |
