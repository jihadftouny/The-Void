...
> **Thirty-nine rows carry `BUG` status** (of **42** — G7, G9 and G10 are `DECIDED`). **Thirty-seven
...
### Found 2026-08-30 by discrepancy pass 10A/11B — the only two findings in otherwise NIL audits
...
| G39 | **A flee consumable ignores `canFlee` — escapes any boss and PERMANENTLY soft-locks Act 5** | `BUG` — **fix specified** | ⛔ | #0a | `resolveUseConsumable` returns `'fled'` without checking `state.canFlee`, which every boss battle sets to `false`. **Measured:** `smoke-vial` in the Act-5 Hollow fight → `status: fled`, back to the hub at act 5. The Hollow is constructed **only** at `act-intro(5)` and `shouldAdvance` is false for act 5, so **the run is stuck grinding random encounters with no path to any ending** — death or Quit only. ⚠ **Honest reachability caveat:** no flee consumable is obtainable today (that is G14), so this goes live the moment G14 is fixed. It is a real defect through the engine API now. **Fix:** `if (res.fled && state.canFlee)`; otherwise consume the item and emit `escape-impossible` |
...
| C6 | **Two different shipped items are both named "Clarity Draught"** | `BUG` — **fix specified** | medium | #13 | `items.json` `clarity-draught` (heal 8) and `consumables.json` `clarity-tonic` (cure `insanity` + apply `wise`) carry the **same display name** and load into the same catalog, so both can sit in one inventory under one name with different behaviour. `GAME-DESIGN.md` §6 documents only the second. It also collides with the `[DECIDED]` rule that **every item gets its own icon** — two icons, one name. **Fix:** rename or delete the `items.json` one; note the survivor's id/name mismatch too |
| C7 | **The combat log prints raw condition IDs, not display names** | `BUG` — **fix specified** | medium | #0c #13 | `render/format.ts:74` emits *"You cannot act — insanity."* and *"**dumb** takes hold of the enemy."* The display names exist in `CONDITION_DATA` and are simply not read. Compounds the two-vocabularies problem: the player sees the **code ids**, which is the vocabulary §14.4 ruled against. *(Currently invisible because `format.ts` is dead code — G18 — so this surfaces the moment G18 is fixed.)* |
