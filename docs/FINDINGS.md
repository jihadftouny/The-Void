...
> **Thirty-two rows carry `BUG` status** (of 35 — G7, G9 and G10 are `DECIDED`). **Twenty-nine have a
> fix specified.** Three do not, for three different reasons:
...
> - **`G32`** — needs a **decision**, not a fix: wire `proficiency` (and re-run the balance sim,
>   because it is a real difficulty change) or delete the field. Leaving it is the one option ruled out.
>
