...
| 2 | **Entrance to the Void** | blinding white, red reflections, distortion — **FRACTURE**: the mind coming apart into pieces that argue (`WORLD.md` §6) | Reflections, Mirror-Selves, Distortions, Static-wraiths | *You can't trust what you see.* Illusory enemies (striking them wastes a turn); **WIS checks** to tell real from false; Mirror-Selves copy your kit. The **clarity↔delusion** floor | **The Reflection** — a mirror of you that fights with your own class's abilities |
| 3 | **The Ash City** | endless grey city, falling ash, silence — **GRIEF**: the ash is what you lost (`WORLD.md` §6 — *"floor 3 is grief, not fear; that reframing is load-bearing"*. It is **not** "emptiness", which is floor 5's register) | **Feelings** (Grief, Rage, Dread, Numbness) and the **Seven Sins** as named elites — all **karma-weighted** | *The ash drains you.* Dampened healing, bleeding resources — attrition mirroring grief. **The endlessness is PROSE, not structure** (§8, `[DECIDED 2026-08-25]`): no variable floor length, no find-the-exit condition | **Your most-indulged Sin/Feeling made flesh** — chosen by your karma, so it's a different, personal fight each run |
...
| 5 | **The True Void** | black, dark, hellish — **ABSENCE**: there was less of you than you thought (`WORLD.md` §6 — *"absence, not destruction"*). The bottom (**only the fallen reach it**) | Demons, Void-horrors (Nightmare), the Unmade, **Echoes of you** (past kills return) | *The Void corrupts your own kit* — skills warp; carried karma directly empowers or punishes you | **Your own Hollow self** — a corrupted mirror wielding your kit and your sins; the culmination of the descent (ties to the Hollow class unlock) |
...
- **Your character's condition** — wounds, what you carry, how the descent is wearing on you. ⚠ **Karma is hidden by design, so a rest may REFLECT who you are becoming but must never STATE it.** A merciful character and a desecrator rest differently — different tone, different detail — with no number ever shown. ⚠ **CORRECTED 2026-09-11 — this entry originally said "the narrator already receives karma in its prompt". That was FALSE.** `buildNarrationPrompt` (`src/llm/narrate.ts`) carries the floor, the act, the facts and recent beats — and **no karma**; line 44 says so. Karma-in-the-prompt is `#12-lite`, scheduled for **v1.1** (`SHIP-SCOPE.md` §9), and was never built. Found by #2's plan agent, not by the orchestrator who wrote the claim. **Consequence: rest spots pull part of `#12-lite` forward into v1** — #2 must build the karma signal for the rest vignette itself, as a closed vocabulary of *manner* words (never an axis name, never a number — G53's rule generalised), and the boss agents (§22.29) will need the same channel. It is new work, not reuse.
