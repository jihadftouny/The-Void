...
| G1, G3–G8, G11–G14, G16–G37, G39–G43 | **Bugs** — fixes specified in FINDINGS §4 (**38 of 40**, counted by script; **there is no G38**). ⚠ **`G2` has no fix yet, and `G15` is an author ruling not a bug.** `G32` was **ruled on 2026-08-30 — wire `proficiency`, then re-run the balance sim** |
