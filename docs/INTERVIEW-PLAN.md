...
| G1, G3–G8, G11–G14, G16–G34 | **Bugs** — fixes specified in FINDINGS §4 (**30 of 32**, counted by script). ⚠ **`G2` has no fix yet, and `G15` is an author ruling not a bug.** `G32` was **ruled on 2026-08-30 — wire `proficiency`, then re-run the balance sim** |
