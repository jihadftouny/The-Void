...
| **#20** | **The stage layout** (`layout-breathing-room`, merged 2026-09-09). Covers **G59 G60 G61**. The choice buttons move out of the reading column into their own column on the right (the author's call), the prose gets a guaranteed eight-line floor, the log a floor as well as a cap, and the scenery is capped by height as well as width with its committed 16:9 governing inside the box. `electron/main.mjs` gains `useContentSize: true`, so the enforced 960×640 minimum is finally the size the PAGE receives rather than the window frame. **The durable part is the guard, not the layout:** `src/dev/layoutProbe.test.ts` measures the real production page in real Chromium at six window sizes and both text sizes, plus a real-boot walk of the actual renderer — because every guard in this area was a source scan or a jsdom assertion and **neither computes layout**. Model and reversals in `UI-DESIGN.md` §17. |
